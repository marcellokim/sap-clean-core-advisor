...
def render_input_form() -> CustomerInput | None:
    ...
    render_section_heading(
        ...
        title=_("Legacy system profile", "Legacy system profile"),
        description=_(
            "현재 SAP ERP 환경과 전환 제약조건을 정리해 분석의 정확도를 높입니다.",
            "Capture the current SAP ERP footprint and transformation constraints to improve assessment quality.",
        ),
    )
    ...
    with st.form("customer_input_form"):
        with st.container(border=True):
            render_section_heading(
                ...
                title=_("기업 정보", "Company profile"),
                description=_(
                    "회사 배경과 전환 일정, 예산 범위를 정리합니다.",
                    "Document the company context, timeline, and investment envelope.",
                ),
            )
            ...
            with col1:
                company_name = st.text_input(
                    _("회사명", "Company Name"),
                    placeholder=_("예: (주)한국제조", "e.g., Contoso Manufacturing"),
                )
                industry_val = st.selectbox(_("업종", "Industry"), _(INDUSTRIES_KO, INDUSTRIES_EN))
                industry = (
                    INDUSTRIES_KO[INDUSTRIES_EN.index(industry_val)]
                    if isinstance(industry_val, str) and industry_val in INDUSTRIES_EN
                    else industry_val
                )

            with col2:
                annual_it_budget = st.number_input(
                    _("연간 IT 예산 (억원)", "Annual IT Budget (100M KRW)"),
                    min_value=0.0,
                    value=50.0,
                    step=5.0,
                )
                migration_timeline = st.slider(
                    _("희망 전환 기간 (개월)", "Desired Migration Timeline (Months)"),
                    min_value=3,
                    max_value=48,
                    value=18,
                )
        ...
        with st.container(border=True):
            render_section_heading(
                ...
                title=_("SAP 시스템 현황", "SAP system status"),
                description=_(
                    "현재 코어 시스템의 버전, 데이터베이스, 사용자 규모를 기록합니다.",
                    "Record the current core system version, database, and usage footprint.",
                ),
            )
            ...
            with col4:
                num_users = st.number_input(
                    _("사용자 수", "Number of Users"),
                    min_value=1,
                    value=800,
                    step=50,
                )
                num_custom_programs = st.number_input(
                    _("커스텀 프로그램(Z-code) 수", "Custom Programs (Z-code) Count"),
                    min_value=0,
                    value=350,
                    step=50,
                )
                custom_code_ratio = st.slider(
                    _("커스텀 코드 비중 (%)", "Custom Code Ratio (%)"),
                    min_value=0,
                    max_value=100,
                    value=45,
                )
        ...
        with st.container(border=True):
            render_section_heading(
                eyebrow="Application footprint",
                title=_("사용 모듈 & 커스텀 심각도", "Modules and customization severity"),
                description=_(
                    "사용 중인 모듈과 커스텀 수준을 기록해 기술 부채 강도를 계산합니다.",
                    "Capture modules in use and customization intensity to estimate technical debt pressure.",
                ),
            )
            st.caption(
                _(
                    "사용 중인 모듈을 선택하고, 각 모듈의 커스텀 수준을 지정하세요.",
                    "Select the modules in use and specify the customization level for each.",
                )
            )
        ...
        with st.container(border=True):
            render_section_heading(
                eyebrow="Transformation priorities",
                title=_("주요 고충사항", "Key pain points"),
                description=_(
                    "운영 리스크와 전환 압박 요인을 입력하면 권고안의 맥락이 더 선명해집니다.",
                    "Describe operating pain points and migration pressures to sharpen the resulting recommendations.",
                ),
            )
            pain_points = st.text_area(
                ...
                placeholder=_(
                    "예: 매월 결산에 5일 이상 소요, 경영진의 클라우드/AI 도입 압박, 시스템 업그레이드 시 커스텀 코드 호환성 문제 빈발...",
                    "e.g., Monthly closing takes 5+ days, executive pressure for Cloud/AI adoption, frequent custom code compatibility issues during upgrades...",
                ),
                height=120,
            )

        st.markdown(
            "<p class='advisor-form-note'>"
            + _(
                "분석 결과에는 Clean Core score, 기술 부채 시각화, TCO 비교, AI 기반 권고안이 포함됩니다.",
                "The assessment returns a Clean Core score, technical debt view, TCO comparison, and AI-backed recommendations.",
            )
            + "</p>",
        ...
        if submitted:
            ...
            if num_custom_programs > 0 and custom_code_ratio == 0.0:
                st.error(
                    _(
                        "커스텀 프로그램 수가 존재하지만 커스텀 코드 비중이 0%입니다.",
                        "Custom program count is > 0 but custom ratio is 0%.",
                    )
                )
                return None
            ...
            if num_custom_programs == 0 and custom_code_ratio > 0.0:
                st.error(
                    _(
                        "커스텀 프로그램 수가 0개인데 커스텀 코드 비중이 0%보다 큽니다.",
                        "Custom program count is 0 but custom ratio is > 0%.",
                    )
                )
                return None
            ...
            if num_custom_programs > num_users * 100:
                st.warning(
                    _(
                        "경고: 사용자 수 대비 커스텀 프로그램 수가 이례적으로 많습니다. 입력값을 확인해 주세요.",
            ...
            if db_size < 10 and num_users > 1000:
                st.warning(
                    _(
                        "경고: 대규모 사용자(1000명 이상) 환경 대비 DB 사이즈(10GB 미만)가 이례적으로 작습니다.",
